...
class AuthService:
    ...
    def login(cls, form_data, headers):
        ...
        _access_token = AuthService.verify_account(username=_username, password=_password, session={
            'xrip': _xrip,
    ...
    def verify_account(cls, username, password, session): 
        ...
        _check_pwd = bcrypt.checkpw(password=password.encode('utf-8'), hashed_password=get(_user, 'password').encode('utf-8'))
