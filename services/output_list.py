...
class OutputListService:
    ...
    def scoring(cls, 
        ...
        result = AnalyticsModel.find_one(filter={"vertical_name":"analytics_default"})
        follower_quality_weight = result["follower_quality_weight"] / 100
    ...
    def get_top_score(cls,login_info):
        ...
        _user = AccountsFilteredDetailModel.find(filter=_filter)
        exclude_filter = {
                        "username": {"$in": [i['username'] for i in _user]},
                        "deleted": False
                        }
